...
// 	if err == nil {
// 		log.Println("Cache hit, returning cached report")
// 		return report, nil // Cache hit, return cached report
// 	}

// 	log.Println("Cache miss, querying database")

// 	// Use a mutex to prevent multiple concurrent requests from executing the same database query
// 	var mu sync.Mutex
// 	mu.Lock()
// 	defer mu.Unlock()

// 	// Double-check if another request has populated the cache while waiting for the lock
// 	err = utils.GetCache(cacheKey, &report)
// 	if err == nil {
// 		log.Println("Cache hit after acquiring lock, returning cached report")
// 		return report, nil
// 	}
...
// 	// Execute each query concurrently using WaitGroup
// 	var wg sync.WaitGroup
// 	for _, q := range queries {
// 		wg.Add(1)
// 		go func(query string, dest **float64) {
// 			defer wg.Done()
// 			args := []interface{}{}
// 			filteredQuery, filteredArgs := injectFilters(query, args)
// 			if err := executeQuery(filteredQuery, filteredArgs, dest); err != nil {
